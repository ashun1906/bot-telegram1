...
# --- CẤU HÌNH WEB SERVER ĐỂ RENDER KHÔNG STOP BOT ---
...
def run_render_web_server():
    # Render sẽ cung cấp Port qua biến môi trường PORT, mặc định là 10000
    port = int(os.environ.get("PORT", 10000))
    server = HTTPServer(('0.0.0.0', port), RenderHealthCheck)
    logging.info(f"--- Web Server phục vụ Render đã chạy tại Port: {port} ---")
    server.serve_forever()

# --- CẤU HÌNH ---
def _parse_tokens():
    raw = os.getenv("BOT_TOKENS") or os.getenv("BOT_TOKEN")
    tokens = []
    if raw:
        try:
            s = raw.strip()
...
OWNER_ID = int(os.getenv("OWNER_ID", "6002553821"))
...
def tick_once():
    try:
        ...
        rows = db_execute("SELECT post_id FROM schedules WHERE time = ?", (now,), fetch=True)
        pids = [r[0] for r in rows]
        if not pids or not TOKENS:
            return 0
        _ensure_worker()
        for token in TOKENS:
            b = Bot(token)
        ...
        try:
            logging.error(f"tick_once error: {e}")
        except:
            pass
        return 0
...
if __name__ == '__main__':
    # 1. Chạy Web Server cho Render ở một luồng riêng
    threading.Thread(target=run_render_web_server, daemon=True).start()

    if '--tick' in sys.argv:
        _ensure_worker()
        count = tick_once()
        print(f"tick: dispatched {count} post(s) for current minute.")
        sys.exit(0)
